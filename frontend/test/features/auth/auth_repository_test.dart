...
class MockDio implements Dio {
  ...
  @override
  Future<Response<T>> post<T>(String path, {data, Map<String, dynamic>? queryParameters, Options? options, CancelToken? cancelToken, ProgressCallback? onSendProgress, ProgressCallback? onReceiveProgress}) async {
...
void main() {
  ...
  group('AuthRepository', () {
    test('login returns UserModel and saves token on success', () async {
      ...
      mockDio.stubPostResponse(Response(
    ...
    test('login throws Exception on Dio failure', () async {
      // Arrange
      mockDio.stubPostError(DioException(
        requestOptions: RequestOptions(path: '/auth/login'),
